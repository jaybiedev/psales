...
$fields = array('account_code','account','address','telefax', 'account_type_id','account_class_id','guarantor_id', 'zero_rated', 'cardname', 
			...
			'date_birth','date_applied','gender','civil_status','no_dependent','salary','employer',
			'terms', 'days_interval', 'grace_period','surcharge','interest_interval','grocery_charge','bond',
			'cutoff1','cutoff2','years_employed','spouse');
...
if ($p1=='Load' && $id!='' )
{
	...
	$q = "select sum(amount) as total_purchase from sales_tender where account_id='".$aaccount['account_id']."'";
	$r = fetch_assoc($q);
	...
	if (!$qr && pg_errno()==1146)
	{
		$qc = "CREATE TABLE `reward` (
			  `reward_id` bigint(20) NOT NULL auto_increment,
			  `date` date default NULL,
			  `account_id` bigint(12) default NULL,
			  `amount` decimal(10,2) default NULL,
			  `points_in` int(11) default NULL,
			  `points_out` int(11) default NULL,
			  `enable` char(1) collate latin1_general_ci default 'Y',
			  PRIMARY KEY  (`reward_id`),
			  KEY `reward_account` (`account_id`)
			) ENGINE=MyISAM DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci";	
		$qcr = @pg_query($qc) or message(pg_errormessage());
		$qr = @pg_query($q);
	}
...
elseif ($p1 == 'Save' && $aaccount['account']!='')
{
	if ($aaccount['account_id'] == '')
	{
		...
		$q = "insert into account (account_code,account, address, telefax, telno, remarks, credit_limit, zero_rated, cardname,
						account_type_id,date_expiry, lifetime, sdisc, cdisc, guarantor_id, account_class_id, cardno)
				values ('".$aaccount['account_code']."','".$aaccount['account']."','".$aaccount['address']."','".$aaccount['telefax']."','".$aaccount['telno']."',
						...
						'".$aaccount['account_class_id']."','".$aaccount['cardno']."')";
	}
	else
	{
...
<form name="f1" method="post" action="" style="margin:0"  enctype="multipart/form-data" >
      ...
      <td colspan="2"> <table width="100%" cellpadding="0" cellspacing="1" bgcolor="#EFEFEF">
          <tr> 
            <td height="18" width="23%"><b><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Name<br>
              <input name="account" type="text" id="account" value="<?= stripslashes($aaccount['account']);?>" size="40" maxlength="40" onFocus="nextfield ='account_code'" onKeypress="if(event.keyCode==13) {document.getElementById('account_code').focus();return false;}" onBlur="document.getElementById('cardname').value=this.value">
              </font></b></td>
            <td height="18" width="10%"><b><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Account 
              # <br>
              <input name="account_code"  onFocus="nextfield ='cardno'" type="text" id="account_code" value="<?= $aaccount['account_code'];?>" size="15" maxlength="15" onKeypress="if(event.keyCode==13) {document.getElementById('cardno').focus();return false;}">
              </font></b></td>
            <td height="18" width="9%"><b><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Card 
              No.<br>
              <input name="cardno"  onFocus="nextfield ='date_applied'" type="text" id="cardno" value="<?= $aaccount['cardno'];?>" size="15" maxlength="15" onKeypress="if(event.keyCode==13) {document.getElementById('date_applied').focus();return false;}">
              </font></b></td>
            <td height="18" width="50%"><b><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Application<br>
              <input name="date_applied"  onFocus="nextfield ='date_expiry'" type="text" id="date_applied" value="<?= ymd2mdy($aaccount['date_applied']);?>" size="15" maxlength="15" onKeypress="if(event.keyCode==13) {document.getElementById('date_expiry').focus();return false;}">
              </font></b></td>
