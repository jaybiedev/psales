...
function round2($num)
{
	...
	$hund = substr($decimal,1,1);
	$tens = substr($decimal,0,1);
	if ($hund <3)
	{
		$hund=0;
	}
...
function nPrinter($pln, $printType=NULL, $dest=NULL)
{
	...
	elseif ($printType=='TCP DRAFT')
	{
	   ...
	   while (true)
	   {
		...
		if (!$fp) 
		{
		 	$rmsg = "No socket connection to printer. After $mtry tries...";
		}
		else
		{	
