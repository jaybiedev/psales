<?
	...
	if (!chkRights2('zread','mview',$ADMIN['admin_id']))
	{
	...
	if ($p1=="Go" || $p1=='Print Draft' ||  $p1=='Print') 
	{
		...
		while ($rt = @pg_fetch_object($qtr))
		{
			...
			$q = "insert into zread (zreadno, date, terminal, current, oldgrand, newgrand, vatsales, nonvatsales, taxamount, lines, admin_id)
						values
								('".$aRep['zreadno']."', '".$aRep['date']."', '".$aRep['terminal']."', '".$aRep['current']."', 
								'".$aRep['oldgrand']."', '".$aRep['newgrand']."', '".$aRep['vatsales']."', '".$aRep['nonvatsales']."', 
								'".$aRep['taxamount']."', '".$aRep['lines']."','".$ADMIN['admin_id']."')";
			$qqr = @pg_query($q) or message(pg_errormessage());
			printZread($aRep, $p1);
	
		
		}
		//$q = "select * from eoday where date='".$aRep['date']."'";
		//$qr = @pg_query($q) or message('Error Posting End of Day : '.pg_errormessage());
		//if (@pg_num_rows($qr) == 0)
		//{
