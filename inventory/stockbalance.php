<?
function getStockBalance($stock_id,$from_date,$to_date,$beg_balance = TRUE){
	$stkled=null;
	$stkled=array();
	
	if($beg_balance){
		$balance_date = date("Y-m-d" ,strtotime("-1 day",strtotime($from_date)));
		$aBeg = getStockBalance($stock_id,'',$balance_date,FALSE);
	...
	$stkled['as_of_date_qty'] = $beg_qty + $balance_qty;
...
function stockBalance($sid, $fdate=null, $tdate=null)
{
	...
	$balance_qty = $in_qty = $out_qty = 0;
	$q = "select case_qty, unit_qty, fraction3, $stockledger.date
		from 
			$stockledger, stock 
		where 
			stock.stock_id = $stockledger.stock_id and
			$stockledger.stock_id='$sid' and 
			$stockledger.date<='$ledgerdate' 
		order by date desc 
			offset 0  limit 1";
	...
	$phycount_date = $r->date;
	if ($r->date != '')
	{
		$fdate = $phycount_date;
	}
	
	if ($r->fraction3 == 0) $fraction3 =1;
	else $fraction3 = $r->fraction3;
	$begqty = $r->case_qty*$fraction3 + $r->unit_qty;
		...
		if ($begqty == 0 && $in_qty == 0) return $stkled;
	...
	$qlr = @pg_query($q) or die (pg_errormessage().$q);
	$rl = @pg_fetch_object($qlr);
	$out_qty = $rl->out_qty;
	...
	$balance_qty = $begqty + $in_qty - $out_qty;
  	...
  	$balance_projected = $balance_available + $po_qty;
		...
		$stkled['phycount_date'] = $phycount_date;
