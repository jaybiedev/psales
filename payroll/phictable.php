...
if ($p1=="Save Checked" && $aphictable['status']=='INSERT')
{
	$c=0;
//	print_r($income_from);
//	echo "here ";exit;
	while ($c < count($mark))
	{
		if ($income_from[$c]!='')
		{
			...
			$q = "insert into phictable (income_from, income_to, employer, employee,  enable)
...
elseif ($p1=="Save Checked" && $aphictable['status']=='LIST')
{
	$c=0;
	...
	while ($c < count($mark))
	{
		...
		{
			pg_query("update phictable set 
						enable='".$enable[$index]."',
						income_from='".$income_from[$index]."',
						income_to = '".$income_to[$index]."',
						employee = '".$employee[$index]."',
						employer='".$employer[$index]."'
					where phictable_id='".$phictable_id[$index]."'") or die (pg_errormessage());
		}
		$c++;
	} 
	$aphictable['status']='SAVED';
...
<form name="form1" method="post" action="">
  <table width="70%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr> 
      <td colspan="6"><font face="Verdana, Arial, Helvetica, sans-serif" size="2">Find 
   ...
   <tr bgcolor="#C8D7E6"> 
      <td colspan="6" height="27" background="../graphics/table_horizontal.PNG" ><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b><font color="#EFEFEF">PHIC 
        Table Setup</font> <a name="top"></a></b></font></td>
    </tr>
    <tr bgcolor="#E9E9E9"> 
      <td width="7%"><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000">#</font></b></td>
      ...
      <td width="13%"><a href="<?=$href.'&sort=income_from';?>"><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000">To</font></b></a></td>
      <td width="17%"><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000">Employer</font></b></td>
      <td width="9%"><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000">Employee</font></b></td>
      <td width="36%"><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
	...
	?>
    <tr bgcolor="#FFFFFF"> 
      <td width="7%" align=right nowrap> <font size=1> 
        <?= $c;?>
      ...
      <td width="13%"><input type="text" name="income_to[]" size="15" id="<?= 'T'.$c;?>"  onChange="vChk(this)" style="text-align:right"></td>
      <td width="17%"><input name="employer[]" type="text"  size="10" id="<?= 'R'.$c;?>"  onChange="vChk(this)" style="text-align:right"> 
      </td>
      <td><input name="employee[]" type="text"  id="<?= 'Y'.$c;?>"  onChange="vChk(this)" size="10" style="text-align:right"> 
      </td>
      <td width="36%"> <select name="enable[]"  id="<?= 'A'.$c;?>"  onChange="vChk(this)">
    ...
    <tr  bgcolor=<?= ($r->enable=='N')? '#FFCCCC' :'#FFFFFF';?>> 
      ...
      <td width="13%"> <input name="income_to[]" type="text"  id="<?= 'T'.$ctr;?>"  onChange="vChk(this)" value="<?= $r->income_to;?>" size="15" style="text-align:right"></td>
      <td width="17%"><input name="employer[]" type="text"  id="<?= 'R'.$ctr;?>"  onChange="vChk(this)" value="<?= $r->employer;?>" size="10" style="text-align:right"> 
      </td>
      <td> <input name="employee[]" employee="text"  id="<?= 'Y'.$ctr;?>"  onChange="vChk(this)" value="<?= $r->employee;?>" size="10" style="text-align:right"> 
      </td>
      <td width="36%"> <select name="enable[]"  id="<?= 'A'.$ctr;?>"  onChange="vChk(this)">
