...
if ($p2=="Save Checked" && $aincome_type['status']=='INSERT')
{
	$c=0;
//	print_r($income_code);
//	echo "here ";exit;
	while ($c < count($mark))
	{
		if ($income_code[$c]!='')
		{
			...
			mysql_query("insert into income_type set
						enable='".$enable[$c]."',
						income_code='".$income_code[$c]."',
						income_type = '".$income_type[$c]."',
						rate = '".$rate[$c]."',
						basis='".$basis[$c]."',
						tax='".$tax[$c]."',
						sss='".$sss[$c]."',
...
elseif ($p2=="Save Checked" && $aincome_type['status']=='LIST')
{
	$c=0;
	...
	{
		$index = $mark[$c]-1;
		if ($income_code[$index]!='')
		{
			$audit = "update:$admin->username:".date('m/d/Y');

			mysql_query("update income_type set 
						enable='".$enable[$index]."',
						...
						phic='".$phic[$index]."',
						audit = '$audit'
					where income_type_id='".$income_type_id[$index]."'") or die (mysql_error());
		}
		$c++;
	} 
	$aincome_type['status']='SAVED';
...
<form name="form1" method="post" action="">
  <table width="39%" border="0" cellspacing="1" cellpadding="2" bgcolor="#CCCCCC" align="center">
    <tr bgcolor="#C8D7E6"> 
      <td colspan="9" height="27"><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b>Income 
    ...
    <tr bgcolor="#E9E9E9"> 
      ...
      <td width="18%"><a href="<?=$href.'&sort=income_code';?>"><b><font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">Code</font></b></a></td>
      <td width="21%"><a href="<?=$href.'&sort=income_code';?>"><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000">Description</font></b></a></td>
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="18%"> <input name="income_code[]" type="text" size="10" maxlength="10" onChange="vChk(this)" id="c<?=$c;?>"> 
      </td>
    ...
    <tr  bgcolor=<?= ($r->enable=='N')? '#FFCCCC' :'#FFFFFF';?>> 
      ...
      <td width="18%"> <input name="income_code[]" type="text" id="income_code[]" value="<?= $r->income_code;?>" size="10" maxlength="10"> 
      </td>
      <td width="21%"> <input name="income_type[]" type="text" id="income_type[]" value="<?= $r->income_type;?>" size="30" maxlength="40"> 
      </td>
      <td width="18%">
