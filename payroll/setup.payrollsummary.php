	...
	while ($ctr < count($mark))
	{
		...
		if ($seqn[$c]!='')
		{
			...
			if ($reportform_id[$c] == '')
			{
				...
				{
					$q = "select * from reportform where  reportform='PAYSUM' and reportform_code = '".$reportform_code[$c]."'";
				...
				$q = "insert into reportform (reportform, enable, reportform_code, title1, title2, type, seqn, long)
						values ('PAYSUM', '".$enable[$c]."','".$reportform_code[$c]."','".$title1[$c]."', '".$title2[$c]."','".$type[$c]."', '".$seqn[$c]."', '".$long[$c]."')";
  ...
  <table width="50%" border="0" cellspacing="1" cellpadding="0" bgcolor="#E9E9E9" align="center">
    <tr bgcolor="#C8D7E6"> 
      <td height="27" colspan="13" background="../graphics/table0_horizontal.PNG"><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b>Setup 
        Payroll Summary Reportform <a name="top"></a></b></font></td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td><input name="reportform_code[]" type="text" id="<?='t'.$c;?>"  onChange="vChk(this)" size="25" maxlength="60"></td>
	...
	$q = "select * from reportform where reportform='PAYSUM' ";
	if ($xSearch != '')
	{
    ...
    <tr  bgcolor=<?= ($r->enable=='N')? '#FFCCCC' :'#FFFFFF';?>> 
      ...
      <td><input name="reportform_code[]" type="text" id="<?='t'.$ctr;?>" onChange="vChk(this)" value="<?= $r->reportform_code;?>" size="25" maxlength="60"></td>
