...
if ($p1=="Save Checked")
{
	...
	while ($ctr < count($mark))
	{
		...
		if ($tender[$c]!='')
		{
			...
			if ($print[$c ] == '') $print[$c ] = 1;

			if ($tender_id[$c] == '')
			{
				$q = "insert into tender (enable, bankable, tender_type, service_grocery, dry_good, tender_code, seq, tender,extra, print)
							values ('".$enable[$c]."','".$bankable[$c]."','".$tender_type[$c]."','".$service_grocery[$c]."','".$service_drygood[$c]."',
							'".$tender_code[$c]."','".$seq[$c]."','".$tender[$c]."','".$extra[$c]."','".$print[$c]."')";
			...
			else
			{
				@pg_query("update tender set
						...
						service_grocery='".$service_grocery[$c]."',
						service_drygood='".$service_drygood[$c]."',
						seq='".$seq[$c]."',
...
<form name="form1" method="post" action="">
  ...
  <table width="50%" border="0" cellspacing="1" cellpadding="2" bgcolor="#E9E9E9" align="center">
    <tr bgcolor="#C8D7E6"> 
      <td height="27" colspan="14" background="../graphics/table0_horizontal.PNG"><font face="Verdana, Arial, Helvetica, sans-serif" size="2"><b>Setup 
    ...
    <tr bgcolor="#E9E9E9"> 
      ...
      <td nowrap><b><font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">Code</font></b></td>
      <td colspan="2" align="center" nowrap><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000">%Service 
        Charge</font></b><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000"><br>
        Grocery </font></b><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000">/ 
        DryGoods</font></b></td>
      <td align="center" nowrap><b></b><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000">Seq</font></b></td>
      <td align="center" nowrap><b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000000">Prints</font></b></td>
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td> 
        <?= lookUpAssoc('tender_type[]',array("Cash"=>"C","Check"=>"K","Bankcard"=>"B","A/R"=>"A","Gift Check"=>"G","Personnel Incentive"=>"P",'Rewards'=>'R'),'');?>
      </td>
      <td><input name="tender_code[]" type="text"  id="<?= 'm'.$c;?>"  onChange="vChk(this)" size="5" maxlength="5"></td>
      <td nowrap><input type="text" name="service_grocery[]" size="4"  onChange="vChk(this)"  id="<?='t'.$c;?>"  > 
      </td>
      <td nowrap><input type="text" name="service_drygood[]" size="4"  onChange="vChk(this)"   id="<?='t'.$c;?>"   ></td>
      <td nowrap> <input type="text" name="seq[]" size="4"  onChange="vChk(this)"  id="<?='t'.$c;?>"  ></td>
      <td nowrap><input type="text" name="print[]" size="4"  onChange="vChk(this)"   id="<?='t'.$c;?>"   ></td>
      <td> 
    ...
    <?
		...
		?>
    <tr bgcolor="#FFFFFF"> 
      <td colspan="12" height="2" valign="bottom">&nbsp;</td>
    </tr>
    <tr bgcolor="#E9E9E9"> 
      <td colspan="10" height="20"><font face="Verdana, Arial, Helvetica, sans-serif" size="2">Saved 
        Categories</font></td>
    </tr>
    <?
	} //if insert
    ...
    <tr  bgcolor=<?= ($r->enable=='N')? '#FFCCCC' :'#FFFFFF';?>> 
      ...
      <td> 
        <?= lookUpAssoc('tender_type[]',array("Cash"=>"C","Check"=>"K","Bankcard"=>"B","A/R"=>"A","Gift Check"=>"G","Personnel Incentive"=>"P",'Rewards'=>'R'),$r->tender_type);?>
      </td>
      <td><input name="tender_code[]" type="text" id="<?='t'.$ctr;?>"  onChange="vChk(this)" value="<?= $r->tender_code;?>" size="5" 
maxlength="40"  ></td>
      <td nowrap><input type="text" name="service_grocery[]" size="4"  onChange="vChk(this)"  id="<?='t'.$ctr;?>" value="<?=$r->service_grocery;?>"> 
      </td>
      <td nowrap><input type="text" name="service_drygood[]" size="4"  onChange="vChk(this)"  id="<?='t'.$ctr;?>"  value="<?=$r->service_drygood;?>"></td>
      <td nowrap> <input name="seq[]" type="text"  id="<?='t'.$ctr;?>"  onChange="vChk(this)" value="<?= $r->seq;?>" size="4"  ></td>
      <td nowrap><input name="print[]" type="text"  id="<?='t'.$ctr;?>"  onChange="vChk(this)" value="<?= $r->print;?>" size="4"  ></td>
      <td> 
	...
	?>
    <tr bgcolor="#FFFFFF"> 
      <td colspan="10" nowrap><font face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
