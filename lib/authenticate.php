<?
	function authenticate($username, $mpassword)
	{
		...
		$p = "login";
		$date_in = $date_out = date('Y-m-d g:ia');
		...
		else
		{
			...
			else
			{
				...
				elseif ($ADMIN['date_expire']< date('Y-m-d') && !in_array($ADMIN['date_expire'],array('','//','--'))) 
				{
				  $q = "insert into userlog (admin_id, date_in, date_out, remarks) 
                  values ('".$ADMIN['admin_id']."','$date_in', '$date_out','User account EXPIRED...Access Denied')";
          		$qr = @pg_query($q) or message(pg_errormessage());
					message('User account EXPIRED... Access Denied.');
					return 0;
				}
				$attempts = 0;
				while ($attempts < 10)
				{
					...
					if (@pg_query($q)) 
					{
						...
						if (pg_num_rows($qa)==0 || !$qa) {
    				  	$q = "insert into userlog (admin_id, date_in, date_out, remarks) 
                      values ('".$ADMIN['admin_id']."','$date_in', '$date_out','Security Check. No Admin Rights.')";
              		$qr = @pg_query($q) or message(pg_errormessage());
							message('Security check...Pls. login again...');
							return 0;
						}
						else {
							$adminrights=array();
