		...
		global $aItems, $aCashier, $ADMIN , $SYSCONF;
			...
			elseif ($f == 'M' && $textbox!='')
			{
				...
				if ($r)
				{
					...
					$msg = "Credit Member: [".$aCashier['cardno'].'] '.$aCashier['account'].'  Points:'.
								$aCashier['points_balance'].
			...
			if (!@pg_num_rows($qr)) 
			{
  					$q    = "select 
  								...
  								from 
  									...
  									barcode
  								where
  									barcode.stock_id=$stocktable.stock_id and
  									$stocktable.enable='Y' and 
   				...
   				if (!$qr) 
    				...
    				if (!@pg_num_rows($qr)) 
    				{
    					if ($SYSCONF['USE_CASE_PRICE']=='Y')
    					{
						   		$q    = "select 
            						...
            						$stocktable
      	   					where
      		    	  				$stocktable.casecode = '$searchitem' and
      		    	  				$stocktable.stock.enable='Y'
                				offset 0 limit 1";					    
					...
					if ($r['fraction'] == 1) //per piece retail
					{
						...
						{
							$q = "select * from stock_price where stock_id = '".$r['stock_id']."'";
							$qr = @pg_query($q);
							if (!$qr)
							{
							...
							$rp = @pg_fetch_assoc($qr);
							$dow = date('w');
							$field = 'price_level'.$dow;
							$price_day = $rp[$field];
...
function altkey($key)
{
	if ($key == 'R' || $key == 'Z' || $key=='B' ||$key=='C' || $key == 'M' || $key == 'G')
	{
...
<body bgcolor="<?= $BODYCOLOR;?>" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onfocus="document.getElementById('textbox').focus()"; onLoad="vOnLoad()" >
  ...
  <table width="99%" height="99%" border="0"	cellpadding="0"	cellspacing="0" align="center">
    <tr	height="5%"> 
      <td width="79%" height="1%" bgcolor="#FFFFFF"> <strong><font color="#CC3300" size="5" face="Times New Roman, Times, serif"> 
        ...
        &nbsp;<img src="../graphics/logo.jpg" width="280" height="60" /> 
        <?= ($_SERVER['REMOTE_ADDR'] == '127.0.0.1' ? 'O-F-F-L-I-N-E  P-O-S' : ($SYSCONF['DATABASE'] != '' ? 'NOTICE: DB = '.$SYSCONF['DATABASE'] : ($SYSCONF['PCENTER']!='' ? strtoupper($SYSCONF['PCENTER']) : '')));?>
        </font></strong></td>
