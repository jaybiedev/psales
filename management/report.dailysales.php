...
if ($p1 == 'Go' || $p1 == 'Print' || $p1 == 'Print Draft')
{
	...
	while ($r = @pg_fetch_object($qr))
	{
		...
		if ($r->units == '0') $units =1 ;
		else $units = $r->units;

		if ($r->mcount == '0') $mcount =1 ;
		else $mcount = $r->mcount;

		if ($r->lines == '0') $lines =1 ;
		else $lines = $r->lines;
		...
		$details .= space(2).adjustSize(ymd2mdy($r->date),10).' '.
					...
					adjustRight(number_format($r->amount/$units,2),8)." ".
					adjustRight(number_format($r->lines/$mcount,2),7)."\n";
					//adjustRight(number_format($r->amount/$lines,2),9)." ".
		$total_count += $mcount;
		$total_days++;
	...
	if (1*$total_count == '0') $total_count = 1;
	if (1*$total_units == '0') $total_units = 1;
	if (1*$total_lines == '0') $total_lines = 1;
...
-->
    <tr> 
      <td height="27" colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2">
          <tr bgcolor="#000033"> 
    ...
    <tr> 
	  ...
	  {
		 echo "<textarea name='print_area' cols='110' rows='20' wrap='OFF' readOnly>$details1</textarea>";
		}	  
		?>
      </td>
    </tr>
