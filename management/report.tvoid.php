...
if ($p1 == 'Print Draft' || $p1 == 'Go')
{
	...
	while ($r = @pg_fetch_object($qr))
	{
		...
		if ($r->status == 'V')
		{
			if ($msales_header_id != $r->sales_header_id)
			{
				...
				$details .= $invoice.'   ';
				$q1 = " select * from admin where admin_id=$r->admin_id";
				$q1r = @pg_query($q1) or message1(pg_errormessage());
				$r1 = @pg_fetch_object($q1r);

				$details .= adjustSize($r1->username,17);
				$details .= adjustRight(number_format2($r->gross_amount,2),12)."\n";
	...
	$details .= adjustRight(number_format2($total_voided,2),12)."\n";
	$details .= "\n\n";
	if ($p1 =='Print Draft')
	{
		nPrinter($details, $SYSCONF['RECEIPT_PRINTER_TYPE'], $SYSCONF['RECEIPT_PRINTER_DEST']);
  ...
  <table width="80%" border="0" align="center" cellpadding="2" cellspacing="0">
    <tr> 
      <td background="../graphics/table0_horizontal.PNG" height="2%"><strong><font size="2" face="Verdana, Arial, Helvetica, sans-serif"> 
        ...
        Voided Transactions </font></font></strong></td>
    </tr>
    <tr> 
      <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif"> Date</font> <font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif"> 
