	...
	WebMasterAlerts=["No such frame: ","Item not defined: ","Item needs height: ","Item needs width: ","Warning-Item doesn't need height: ","Warning-Item doesn't need width: ","Item Ok ","Menu tree ok"];
...
function Go(){
	...
	if(FirstCreate){
		...
		if(WebMasterCheck){if(!Check('go',NoOfFirstLineMenus))return;else WebMasterAlert(7,'')}
	...
	if(FirstCreate){FirstContainer=CreateMenuStructure('Menu',NoOfFirstLineMenus);FirstCreate=0}
	...
	status='Welcome to HOPE ePOS Information Systems'}

function CalcLeft(){
	var Size=FirstWindowWidth;
	if(MenuCentered!='left'){
		StartLeft=M_StartLeft;
		Size-=(FirstLineHorizontal)?(NoOfFirstLineMenus*(eval('Menu1[4]')+BorderWidth)+BorderWidth):(eval('Menu1[4]')+BorderWidth);
...
function CalcTop(){
	...
	if(MenuVerticalCentered!='top'){	
		StartTop=M_StartTop;
		Size-=(FirstLineHorizontal)?(eval('Menu1[3]')+BorderWidth):(NoOfFirstLineMenus*(eval('Menu1[3]')+BorderWidth)+BorderWidth);
		if(MenuVerticalCentered=='bottom')StartTop+=Size; else StartTop+=Size/2}}

function PositionMenuStruct(CntnrPntr,Tp,Lt){
	if (CntnrPntr == null){return};
...
function ClearAllChild(Pntr,ChldPntr){
	var CPCCStyle;
	while (Pntr){
		if(Pntr.ChildCntnr){
			CPCCStyle=(Nav4)?Pntr.ChildCntnr:Pntr.ChildCntnr.style;
			if(Pntr.ChildCntnr!=ChldPntr)CPCCStyle.visibility=M_Hide;
			ClearAllChild(Pntr.ChildCntnr.FirstMember,ChldPntr)}
...
function OpenMenu(){
	...
	if(ShowFlag){
		ClearAllChild(this.Container.FirstMember,ChildCont); 
	...
	if(ChildCont!=null)	{
		...
		ShowFlag=1}
	if (ExpYes){status=this.innerHTML;}
