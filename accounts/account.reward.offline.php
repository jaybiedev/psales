...
<?
if (!chkRights2('rewardsoffline','mview',$ADMIN['admin_id']))
{
		message1("OFFLINE REWARDS ENCODING NOT ALLOWED. PLEASE ASK SYSTEMS ADMINISTRATOR");
		exit; 
}
...
elseif ($p1 == 'Ok')
{
    ...
    if (in_array($aReward['tender_type'], array('C','K')))
	{
	  ...
	  if ($term['AREA_ID'] == '1')
	  {
		$reward_grocery = round($aReward['amount_in']/$SYSCONF['CASH_GRC_POINT'],2);
	  }
	  else
	  {
		$reward_dry = round($aReward['amount_in']/$SYSCONF['CASH_DRY_POINT'],2);
	  }
	}
	...
	$points_in = round($reward_grocery + $reward_dry,2);
/*
	if ($points_in < 1)
	{
...
*/
	...
	{
		$q = "insert into reward (date, invoice, type, terminal, tender_id, 
						amount_in, account_id, points_in, admin_id)
					values  ('".$aReward['date']."',
						'".$aReward['invoice']."',
						'2',
						'".$aReward['terminal']."',
  ...
  <table width="90%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
    ...
    <tr bgcolor="#EFEFEF"> 
      <td width="6%"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Date</font><br> 
        <input name="date" type="text" class="altText" id="date" onKeypress="if(event.keyCode==13) {document.getElementById('invoice').focus();return false;}" value="<?= ymd2mdy($aReward['date']);?>" size="10" onBlur="f1.action='?p=account.reward.offline&p1=changedate';f1.submit()"> 
      </td>
