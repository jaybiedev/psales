...
function customerBalance($aid)
{
  ...
  /*$q = "select type, debit, credit, date
        from 
          accountledger
        where
          account_id='$aid' and 
     ...
     /*removed offset 0 limit 5, dont know why he is trying to limit*/
...
$q = "select type, debit, credit, date
 ...
 ";
  ...
  {
  	if ($date_lasttrans == '') 
  		$date_lasttrans = $R['date'];
  	elseif ($R['date'] < $date_lasttrans)
  	{
  		break;
  	}	
  	if ($R['type'] == 'T') 
  	...
  	elseif ($R['type'] == 'I')
  	{
  		$overdue += ($R['debit'] - $R['credit']);
  	}
  	elseif ($R['type'] == 'P')
  	{
  		$overdue -= ($R['credit'] - $R['debit']);
  	}
...
//				$interest += $rr->debit - $rr->credit;
			...
			else
			{
				...
				if ($rr->grocery_cutoff1 == '0')
				{
					...
					/*
					 * GET DUE DATE BY ADDING INTERVAL
					 */
					
					$date = date("Y-m-d", strtotime($rr->date));
					$date_due = date("Y-m-d",strtotime("+$interval days",strtotime($date)));

					// REMOVED CODES STARTS HERE
